include:
  - openstack.cinder.cinder_install
  - openstack.cinder.cinder_config
...
cinder-keystone-init:
  cmd.run:
    - name: bash /usr/local/bin/cinder_data.sh && touch /var/lock/cinder-datainit.lock
    - require:
      - file: /usr/local/bin/cinder_data.sh
    - unless: test -f /var/lock/cinder-datainit.lock

cinder-db-init:
  cmd.run:
    - name: cinder-manage db sync && touch /var/lock/cinder-db-sync.lock
    - require:
      - mysql_grants: cinder-mysql
    - unless: test -f /var/lock/cinder-db-sync.lock
